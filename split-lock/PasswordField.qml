...
BorderSurface {
  ...
  LockInput {
    ...
    // PORT (split-lock): the left inset only has to clear the lock glyph.
    // Upstream used max() of both reserves, which borrowed the right-hand
    // icons' width (eye + face) for the left side and pushed the text ~65px
    // right of the box's own padding.
