...
| `yvonne.workspaces.patch` | Dynamic workspace pill count (`1..N` by occupancy, never below 2) |
...
`yvonne.arch-logo`, `yvonne.workspaces` and `yvonne.split-lock` are ours, have no `.git`, and
